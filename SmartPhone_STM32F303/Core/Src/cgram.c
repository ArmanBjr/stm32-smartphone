...
/* BANK_LOGO: 2 adjacent glyphs forming a simple bracket/phone-body
 * silhouette, printed side by side on the boot screen. Indices exposed
 * via cgram.h so phone.c knows which custom-char codes to print. */
static const uint8_t s_logo_glyphs[2][8] = {
  { 0x07, 0x08, 0x10, 0x10, 0x10, 0x10, 0x08, 0x07 }, /* left rounded edge */
  { 0x1C, 0x02, 0x01, 0x01, 0x01, 0x01, 0x02, 0x1C }, /* right rounded edge */
};
...
/* BANK_MUSIC (Phase 5, plan section 3's "3-state volume icon"): mute
 * (crossed-out speaker), low (speaker, one sound wave), high (speaker, two
 * sound waves) -- same hand-drawn-icon design footing as the menu icons
 * above (this file's own header comment). Only 3 of 8 slots used; the
 * progress bar deliberately uses the LCD's built-in 0xFF block char
 * instead of a 4th custom glyph (plan risk register item 2: CGRAM budget). */
static const uint8_t s_music_icons[3][8] = {
  { 0x00, 0x01, 0x03, 0x1F, 0x03, 0x01, 0x00, 0x15 }, /* mute: speaker + "x" dots */
  { 0x01, 0x03, 0x07, 0x1F, 0x07, 0x03, 0x01, 0x00 }, /* low: speaker, no waves */
  { 0x01, 0x03, 0x07, 0x1F, 0x07, 0x03, 0x01, 0x0A }, /* high: speaker + wave dots */
};
